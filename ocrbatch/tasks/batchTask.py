...
import os
import json
from utils.batchfile import process_batch_task
...
async def process_task(user_id, id, task):
    ...
    async with job_semaphore:
        ...
        try:
            ...
            # await process_batch_task(user_id,task['nSectionid'], task['nSectionid'],, task['nSectionid'], task['nSectionid'])

            redis_client.delete(f"batch_download:{user_id}:{id}")  # Discard task after completion
