-- 2026-06-09 — et_bundle_index: a doc-OFFSET branch (find-result navigation)
--
-- Find results are ranked, not in index order, and a match's folder can hold
-- thousands of docs (e.g. tab "J44" is a DIRECT doc of "Section J", 2,695 rows
-- past the folder start). Navigating to the FOLDER therefore lands far from the
-- document. This branch (taken when `nFindDocid` is set) returns the document's
-- exact 0-based position in the gated, sorted_tab body order, so the client can
-- jump to its page and scroll to its row. Computed once per click (not per
-- search hit), so the O(position) count is cheap enough.
--
-- The OUTLINE / SEARCH / BROWSE branches are byte-identical to
-- 2026-06-09_bundle_index_ranked_search.up.sql.
--
-- Depends on: 2026-06-09_bundle_index_ranked_search.up.sql
-- Rollback:   2026-06-09_bundle_index_doc_offset.down.sql
