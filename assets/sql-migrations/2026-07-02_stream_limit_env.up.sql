-- 2026-07-02 — download.et_get_approximate_size: ops-tunable stream cutover
-- The direct-stream vs queued-job threshold was hardcoded at 1 GiB. The
-- download app now passes nStreamLimit (from DOWNLOAD_STREAM_LIMIT_BYTES)
-- and the SP uses it as the fallback when the case has no CaseMaster.cDSize.
-- Priority: per-case cDSize > nStreamLimit (env) > 1 GiB default. Additive.
