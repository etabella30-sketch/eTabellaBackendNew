-- 2026-06-30 — download.et_insert_download_process_files_hyperlink
-- Prefix every downloaded document title with its tab reference ("cTab").
--   * main bundle docs   : separator changed from space -> dash, plus dedup guard
--   * highlight/hyperlink docs (the "hyperlink doc/" rows): tab prefix ADDED (was missing)
-- Dedup guard: if the title already starts with the tab reference, do NOT prefix again.
-- Result: "<cTab>-<title>", e.g. "1-Cast Form Abnormal.pdf".
-- Additive / backward-compatible (only the cFilename column written to ProcessBatchs changes).
	...
	if(jsonb_array_length(jFolders) = 1) then
	  ...
	  SELECT t."nBundledetailid",t."cPath",t."foldername",t."cFilename"
        FROM (
			...
			union all
				SELECT DISTINCT bd."nBundledetailid",bd."cPath", COALESCE(tm.Foldername, '') || 'hyperlink doc/' as Foldername,
				(case when coalesce(bd."cTab",'') = ''
				       or left(lower(btrim(bd."cFilename")), length(btrim(bd."cTab"))) = lower(btrim(bd."cTab"))
				       then '' else bd."cTab" || '-' end)
				|| (CASE  WHEN upper(bd."cFilename") LIKE ('%' || ('.' || upper(substring(bd."cPath" FROM '.*\.([^.]+)$'))))  THEN regexp_replace(bd."cFilename", '[\/\\\""\''''\:?\<\>\n\r]', '', 'g') ELSE  regexp_replace(bd."cFilename", '[\/\\\""\''''\:?\<\>\n\r]', '', 'g') || ('.' || upper(substring(bd."cPath" FROM '.*\.([^.]+)$')))
		  ...
		  SELECT t."nBundledetailid",t."cPath",t."foldername",t."cFilename" from
					( with tm as (SELECT bd."nBundledetailid","cPath",'/' ||  (CASE  WHEN COALESCE(bd."cTab", '') != '' THEN COALESCE(bd."cTab", '')   ELSE left(regexp_replace(bd."cFilename", '\.[^.]*$', ''),100)  END) || '/' as Foldername,
					(case when coalesce(bd."cTab",'') = ''
					       or left(lower(btrim(bd."cFilename")), length(btrim(bd."cTab"))) = lower(btrim(bd."cTab"))
					       then '' else bd."cTab" || '-' end)
