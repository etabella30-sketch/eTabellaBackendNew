-- Rollback of 2026-06-09_bundle_index_ranked_search.up.sql
-- Removes the ranked SEARCH branch, restoring the two-branch function from
-- 2026-06-09_bundle_index_body_count.up.sql (outline + browse only).
-- NOTE: with this rolled back, find reuses the browse filter again (cSearch on
-- the body), so the frontend must also revert to filtering — keep them in sync.
