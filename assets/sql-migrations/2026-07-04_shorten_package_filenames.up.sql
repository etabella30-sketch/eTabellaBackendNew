-- 2026-07-04  Cap packaged filenames at 80 chars (was 150) so extract
-- paths stay under Windows MAX_PATH (260). Long doc names + a deep
-- Downloads folder pushed some files past 260 chars; Explorer/zip tools
-- skip or truncate those on extract, so the Master Index links (which use
-- the full name) 404. The index href uses the SAME ProcessBatchs.cFilename,
-- so shortening keeps index and file matched; the full description still
-- shows in the index table column. Tab prefix keeps names unique.
