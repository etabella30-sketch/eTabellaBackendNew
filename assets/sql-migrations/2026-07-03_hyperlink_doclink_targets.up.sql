-- 2026-07-03  Linked-bundle packages also follow reader DocLink marks
-- (docs: eTabella angular 21/docs/reader-export-plan.md §Phase C, L4/G6).
--
-- Until now the hyperlink SPs only read the legacy auto-generated
-- "HyperLink"+"Annotations" rows; links a user creates in the reader
-- (DocMaster -> DMLinks, visual rects in "Annotations" keyed by nDocid)
-- were neither packaged nor rewritten. Two changes:
--
-- 1) download.et_insert_download_process_files_hyperlink additionally pulls
--    each picked doc's DMLinks targets into '<source folder>/hyperlink doc/'
--    (same cleaned-filename expression as the HyperLink branch), in BOTH
--    selection halves (folder tree + loose jFiles). The final insert is now
--    DISTINCT — a target linked via both mechanisms (or twice) must land in
--    the archive once.
--
-- 2) download.et_get_hyperlink_jobs merges HyperLink + DocLink entries into
--    ONE metadata row per source PDF. (Two rows per doc would queue two
--    python rewrites of the same file; the second upload would overwrite the
--    first and silently drop its links.) DocLink hotspot rects come from the
--    "Annotations" rows whose "nDocid" is the DocMaster PK ("nHLid" IS NULL
--    guards against the overloaded column); link_text is the target's cTab.
--    Multi-destination DocLinks produce stacked links on the same rect —
--    viewers resolve the topmost; exact per-destination page targeting
--    (DMLinks link-scope JSON -> rect[0].redirectpage) is a follow-up.
