-- 2026-07-01 — download."ProcessMaster"."jInclude" + insert SP stores it
-- Persists the Outputs "Download Case Package" include-flags (evidence/
-- transcripts/facts/tags/doclinks/metadata + A–K sections) with the job, so the
-- package builder can gate document inclusion and inject generated data reports.
-- et_process_detail already does `select p.*`, so it returns jInclude without change.
-- Additive: new nullable column + CREATE OR REPLACE of the insert SP (jInclude read + stored).
