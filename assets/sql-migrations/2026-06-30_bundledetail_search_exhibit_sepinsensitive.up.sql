-- 2026-06-30 — Separator-insensitive exhibit-number search (up)
--
-- PROBLEM
...
-- Expression trigram GIN matching the new predicate's left operand so substring
-- exhibit search stays index-assisted. Mirrors the inner shape of the existing
-- ix_bd_exhibit_trgm (lower(coalesce(...::text))) wrapped in the same
-- regexp_replace the predicate applies.
CREATE INDEX CONCURRENTLY IF NOT EXISTS ix_bd_exhibit_norm_trgm
  ON public."BundleDetail" USING gin (
    regexp_replace(lower(coalesce("cExhibitno", ''::character varying)::text), '[^a-z0-9]', '', 'g') gin_trgm_ops
  );
