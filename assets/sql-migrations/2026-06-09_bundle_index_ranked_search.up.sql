-- 2026-06-09 — et_bundle_index: a ranked SEARCH branch (find-in-document)
--
-- The viewer's "find" used to reuse the browse body (filter + ORDER BY sorted_tab),
-- so an exact tab like "R1" was buried — it sorts late, past the page cap, while
-- name-matches ("…R1_ARC.xlsx") filled the top. Find now runs as a SEPARATE call
-- and the index body is no longer filtered/hidden.
--
-- This adds a dedicated branch (taken when cSearch <> '', which now ONLY the find
-- uses — the browse always passes cSearch = '') that RANKS matches by field
-- priority so the relevant doc surfaces first:
--   tab exact (6) > tab prefix (5) > tab contains (4)
--     > exhibit (3) > document name (2) > date (1).
-- Searchable fields: cTab, cExhibitno, cFilename, dIntrestDt (the trigram GIN
-- indexes ix_bd_*_trgm make the ILIKE filter fast; measured ~220ms on the 77k-doc
-- section, date adds ~15ms). count(*) OVER() is fine here — the filter narrows to
-- the (small) match set before the window.
--
-- The OUTLINE and BROWSE branches are byte-identical to
-- 2026-06-09_bundle_index_body_count.up.sql.
--
-- Depends on: 2026-06-09_bundle_index_body_count.up.sql
-- Rollback:   2026-06-09_bundle_index_ranked_search.down.sql
