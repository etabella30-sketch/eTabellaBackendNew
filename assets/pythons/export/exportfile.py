...
def draw_image(annotpages, page, type, maxmin, pgwidth, pageNo):
    ...
    try:
        startX = (pgwidth - extraWidth) - 20
        startY = maxmin['frm'] - 10 if type == 'D' else maxmin['frm']
        endX = (pgwidth - extraWidth) - 20
...
def drow_ract(annotpages,x,page,rotation):
     if x["rects"] and len(x["rects"]):
        ...
        draw_image(annotpages, page, x["linktype"], maxmin, pgwidth, x["page"])
...
fact_img, doc_img, web_img, factq_img = f'{os.getenv('ROOT_PATH')}assets/icon/linksicon/fact.png',f'{os.getenv('ROOT_PATH')}assets/icon/linksicon/doc.png',f'{os.getenv('ROOT_PATH')}assets/icon/linksicon/web.png', f'{os.getenv('ROOT_PATH')}assets/icon/linksicon/qfact.png'
...
logging.basicConfig(
    filename=f'{os.getenv('ROOT_PATH')}logs/{lgfile}/export/{str(data["nEDid"])}.log',  # Specify the log file name
