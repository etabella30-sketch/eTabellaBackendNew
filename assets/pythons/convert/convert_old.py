...
log_dir = f'{os.getenv('ROOT_PATH')}logs/convert/'
lgfile = get_date()
...
def convert_doc_to_pdf_linux(input_file, output_file, port=2002):
    ...
    # Create unique temporary directory for this conversion
        temp_dir = f"/tmp/LibO_Process_{port}"
...
def convert_xls_to_pdf_linux(input_file, output_file, port=2002):
    try:
        ...
        port = int(port)
        # Start LibreOffice listener dynamically
        # if not start_libreoffice_listener(port):
        #     raise RuntimeError(f"Failed to start LibreOffice listener on port {port}")

        # port = find_free_port()  # Find a free port
   # Check if the listener is active
        # if not is_port_open(port):
        #     print(f"Port {port} is not active. Attempting to start listener...")
        #     start_unoconv_listener(port)
        # ensure_listeners_running([port])  # Check and start the listener if needed

        response("Progress: 10.2")        
        response(f"port : {port}")        
        
        logging.info(f"Starting conversion of {input_file} to {output_file}")
        
        responce(f"Step 2")
        # Corrected command arguments for unoconv
        ...
        #     ["unoconv", f"--port={port}", "-f", "pdf", "-o", pdf_path, docx_path],
        #     capture_output=True,
        #     text=True  # Ensure stdout and stderr are returned as strings
        # )

        responce(f"Step 3")
        # Read and log output while the process is running
        while True:
            output = process.stdout.readline()
            if output == '' and process.poll() is not None:
                responce(f"Step 3.1 ")
            ...
            if output:
                ...
                logging.info(f'Progress: 50 - {output.strip()}')

        responce(f"Step 4")
        # Capture and log any stderr output after the process completes
        stderr = process.communicate()[1]
        if stderr:
        ...
        logging.info(f'Successfully converted Excel to PDF - {output_file}')
        response(f"Successfully converted Excel to PDF: {output_file}")
    except subprocess.CalledProcessError as e:
        logging.error(f'Error converting Excel to PDF: {e}')
        response(f"Error converting Excel to PDF: {e}")
        sys.exit(1)
        return 1
    except Exception as e:
        logging.error(f'Unexpected Error converting Excel to PDF: {e}')
...
def convert_to_pdf(input_file, output_file,port = 2002):
    ...
    try:
        ...
        logging.info(f'convert start form file  {input_file}')
        if file_extension in ['.doc', '.docx']:
            ...
            if platform.system() == 'Windows':
                ...
                convert_doc_to_pdf_linux(input_file, output_file,port)
            ...
            if platform.system() == 'Windows':
                ...
                convert_ppt_to_pdf_linux(input_file, output_file,port)
