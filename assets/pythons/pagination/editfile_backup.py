...
lgfile = get_date()
logging.basicConfig(
    filename=f'./logs/pagination/{lgfile}.log',  # Specify the log file name
...
def add_footer_polygon(doc, start_page, end_page, footer_text, font_size):
    # Open the PDF
...
def insert_footer_parallel(pdf_path, footer_text, font_size=10, batch_size=20):
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for start_page in range(0, total_pages, batch_size):
                ...
                future = executor.submit(add_footer_polygon, doc, start_page, end_page, footer_text, font_size)
...
sourcePath = './filebackup/'
outputPath = '/usr/src/app/assets/'
...
if "cRefpage" in data and "cTab" in data and data["cRefpage"] and len(data["cRefpage"]) > 0 and len(data["cTab"]) > 0:
    ...
    logging.info(f'data match {data["cTab"]}  {data["cRefpage"]}')
    ...
    insert_footer_parallel(data["cPath"], footer_text, font_size)    
