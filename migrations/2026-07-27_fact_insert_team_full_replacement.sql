-- 2026-07-27 — Fact share edit: removals never applied
--
-- Symptom: reducing a Fact's "Share with my team" list and saving left every
-- previously-shared member selected on reopen.
--
-- Root cause: realtime.et_fact_insert_team's replacement DELETE was scoped
--   AND f."nShareBy" = nUserid
-- but FMShared rows created by public.et_fact_insert_team (coreapi path) and
-- the legacy realtime.et_fact_insert_team_old carry nShareBy NULL (530 of 1283
-- rows on dev at time of writing), and rows created by a different resharer
-- carry their id — none of those can ever match the editor's id, so they
-- survived every save. The share editor (realtime.et_factsheet_shared) shows
-- and edits ALL FMShared rows for the fact regardless of sharer, so the saved
-- jUsers list is the user's intended FULL replacement — the delete must match
-- that scope (this is also what the legacy commented-out sync block in
-- et_factsheet_submit did: no sharer scoping).
--
-- Change: drop the nShareBy predicate from the DELETE (full replacement), and
-- refresh nShareBy on conflict so surviving rows converge to a real sharer
-- instead of staying NULL forever.
--
-- Apply: dev etabella_tech_uuid first; prod etabella.com.uuid at deploy
-- (together with the realtime-server restart for the 2026-07-27 DTO fixes).

DO $$ BEGIN RAISE NOTICE 'Applying et_fact_insert_team full-replacement fix on %', current_database(); END $$;
