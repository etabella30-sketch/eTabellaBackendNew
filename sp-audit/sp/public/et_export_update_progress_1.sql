CREATE OR REPLACE FUNCTION public.et_export_update_progress_1(parameter json, ref refcursor)
 RETURNS refcursor
 LANGUAGE plpgsql
AS $function$
...
update "ExportDetail" set "nProgress" = nProgress,
		"cStatus" = (case
						when cStatus = 'C' then 'C'
						when cStatus = 'F' then 'F'
						when cStatus in ('P','S','L','I','M','R','U') then 'I'
						else "cStatus"
						end
					),
		...
		"dCompDt" = (
					case
						-- when cStatus = 'C' then now()
						when cStatus IN ('C', 'F') then now()
						else "dCompDt"
					end
					)
...
if exists(select * from "ExportMaster" where "nExportid" = nExportid and "cType" = 'S')then
...
IF EXISTS (
        SELECT 1 FROM "ExportDetail"
        WHERE "nExportid" = nExportid AND "cStatus" = 'F'
        ) THEN
        UPDATE "ExportMaster"
        SET "cStatus" = 'F'
