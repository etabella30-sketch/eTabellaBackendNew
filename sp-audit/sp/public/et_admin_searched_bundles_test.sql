CREATE OR REPLACE FUNCTION public.et_admin_searched_bundles_test(parameter json, ref refcursor)
 RETURNS refcursor
 LANGUAGE plpgsql
AS $function$
DECLARE
    ...
    WHERE case when (
	    ...
	    then (
	 	...
	 	select cr."nBundledetailid"
			...
			where '  || (case when filter_string is not null then (' (' || filter_string || ') AND') else '' end)  || '
			(f."nUserid" = ''' || nMasterid || '''::uuid or bt."nUserid" = ''' || nMasterid || '''::uuid  or idl."nUserid" = ''' || nMasterid || '''::uuid or ifs."nBundledetailid" = cr."nBundledetailid")
			 -- (f."nFSid" is not  distinct from null  or  bt."nBDTsid" is not  distinct from null)
