CREATE OR REPLACE FUNCTION public.et_bundledetail_test(parameter json, ref1 refcursor)
...
-- select * from public.et_bundledetail_test ('{"nSectionid":"e38b6eff-b7dc-43cb-a4f5-8253e696dec6","nBundleid":"b111023e-078c-47f4-bbf8-3a07ae037e10","pageNumber":1,"cSearch":"","cFiletype":"ALL","cSortby":"cTab","cSorttype":"ASC","nStarttabid":null,"nEndtabid":null,"jFilter":"[{\"name\":\"LINK\",\"type\":\"V\",\"value\":[\"I\"]}]","nMasterid":"ba561c55-81f5-4180-8934-2ce6dcaa096c"}','r1');fetch all in "r1";
    ...
    childOrder AS (
        SELECT ROW_NUMBER() OVER (ORDER BY sorted_tab) range_serial,ROW_NUMBER() OVER (ORDER BY 		
    ...
    ranges AS (
        SELECT 
            ...
            END || ' THEN range_serial END) AS s_serial,
            MAX(CASE WHEN ' || 
            CASE WHEN nEndtabid IS NULL THEN 'false' 
                ELSE '"nBundledetailid" = ''' || nEndtabid || '''::uuid' 
            END || ' THEN range_serial END) AS e_serial
        FROM childOrder
    )
    SELECT range_serial,serial,ranges.s_serial,ranges.e_serial,"nBundledetailid","nBundleid","cName","cTab","cExhibitno","cBundletag",
           "cPage","cRefpage","cFilesize", "cFiletype","dIntrestDt","cDescription", "cIsindex","cAuthor","cPageRange"
    FROM childOrder CROSS JOIN ranges 
    WHERE (ranges.s_serial IS NULL OR childOrder.range_serial >= ranges.s_serial) AND (ranges.e_serial IS NULL OR childOrder.range_serial <= ranges.e_serial)
    ORDER BY serial
