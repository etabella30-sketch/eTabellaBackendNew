CREATE OR REPLACE FUNCTION public.filter_whereclause_3(jfilters jsonb, ctype text)
 RETURNS text
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    ...
    FOR i IN 0 .. jsonb_array_length(jfilters) - 1 LOOP
                    ...
                    if(jsonb_array_length(filter_values) > 0) then
						if(filter_name = 'LINK') then
							...
							if(filter_values ? 'O') then 
	                        	result := result || (case when filter_values ? 'I' then  ' OR ' else '' end) || 'f."nFSid" is not null or idl."nDocid" is not null' ;				   
							end if;
						else
		...
		ELSIF filter_name = 'TASK' and ctype = 'FILES'  then
   			 BEGIN
		        ...
		        -- jPriority
			        ...
			        IF jsonb_array_length((filter_values->>'Timeline')::JSONB) > 0 THEN
			            task_conditions := task_conditions || '(' ||
			                ...
			                '((td."jTimeline"->>''dEnd'')::date - CURRENT_DATE) IN (' ||
