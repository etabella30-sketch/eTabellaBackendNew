CREATE OR REPLACE FUNCTION public.try_convert_to_dates_end_only(input_text text)
 RETURNS date
 LANGUAGE plpgsql
 IMMUTABLE
AS $function$
...
--             end_date := NULL;
	...
	-- raise notice 'temp_input %  input_text %',temp_input,input_text;
		...
		end_date := (date_trunc('MONTH', to_date(regexp_replace(input_text, '(^\d{1,2})+\s*-+\s*(\d{1,2})+\s+(\d{4})$', '\2 \3', 'g'), 'MM YYYY'))   + INTERVAL '1 MONTH' - INTERVAL '1 day')::date;
-- 		EXCEPTION WHEN OTHERS THEN
--             -- If direct conversion fails, return NULLs
--             start_date := NULL;
--             end_date := NULL;
...
EXCEPTION WHEN OTHERS THEN
            ...
            end_date := NULL;
        END;
--         start_date := 'asdf';-- temp_start_date ;--to_date(coalesce(temp_start_date,''), 'DD Month YYYY');
--         end_date := 'asdf';-- temp_end_date;-- to_date(coalesce(temp_end_date,''), 'DD Month YYYY');
