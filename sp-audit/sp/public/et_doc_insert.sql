CREATE OR REPLACE FUNCTION public.et_doc_insert(parameter json, ref refcursor)
 RETURNS refcursor
 LANGUAGE plpgsql
AS $function$
DECLARE nDocid uuid;
...
nPMid := (select "nPMid"  from "PermissionModule" where "cType" = 'NF' );
	...
	insert into "DocMaster" ("nBundledetailid","nUserid","dCreateDt","nCaseid")
	values(nBundledetailid,nMasterid,now(),nCaseid)
	RETURNING "nDocid" INTO nDocid;
	
	insert into "DocDetail" ("nDocid","cType","jLinktype","jOText")
	values(nDocid,cType,jLinktype,jOT);
...
FROM jsonb_to_recordset(rec.annot) AS ( "uuid" text, "type" text, "rects" jsonb, "lines" jsonb, "width" int, "colorid" uuid, "page" int
	...
	SELECT nDocid,NULLIF(jsonb_array_elements_text(jTeams)::text,'')::uuid AS i;
