CREATE OR REPLACE FUNCTION public.fsym_on_u_for_pblc_srprmssn_trg_lv_grp()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$                                                                                                                
          ...
          case when new."nOUPid" is null then '' else '"' || cast(cast(new."nOUPid" as numeric) as varchar) || '"' end||','||
