CREATE OR REPLACE FUNCTION public.et_navigate_factlinks(parameter json, ref1 refcursor, ref2 refcursor, ref3 refcursor)
...
/*
     factids :=  (array ( SELECT f."nFSid"
    FROM "FactMaster" f
    LEFT JOIN "FMShared" fs ON fs."nFSid" = f."nFSid"
    LEFT JOIN "FMLinks" fl ON fl."nFSid" = f."nFSid"
    WHERE (f."nBundledetailid" = nBundledetailid OR fl."nBundledetailid" = nBundledetailid)
      AND (f."nUserid" = nMasterid OR fs."nUserid" = nMasterid)
    GROUP BY f."nFSid", fl."nFMLid", f."dCreateDt"
