CREATE OR REPLACE FUNCTION public.et_contact_rolebuilder(parameter json, ref refcursor)
 RETURNS refcursor
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    ...
    IF cPermission = 'N' THEN
        IF NOT EXISTS (
            ...
            OPEN ref FOR SELECT 1 AS msg,'Occupation Inserted' AS value,nCRoleid AS "nCRoleid";
        ELSE
            OPEN ref FOR SELECT -1 AS msg,'Occupation Already Exists' AS value;
        END IF;
    ...
    ELSIF cPermission = 'E' THEN
        IF NOT EXISTS (
            ...
            OPEN ref FOR SELECT 1 AS msg,'Occupation Updated' AS value,nCRoleid AS "nCRoleid";
        ELSE
            OPEN ref FOR SELECT -1 AS msg,'Occupation Already Exists' AS value1;
        END IF;
