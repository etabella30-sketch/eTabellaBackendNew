CREATE OR REPLACE FUNCTION public.et_download_change_serial(parameter json, ref refcursor)
 RETURNS refcursor
 LANGUAGE plpgsql
AS $function$
...
nDTaskid:= NULLIF(parameter ->>'nDTaskid','')::uuid;
	...
	update "DownloadTDetail" set "nSerial" = case when fIndex > nIndex then "nSerial" - 1 else "nSerial" + 1  end where "nDTaskid" =nDTaskid and "nBid" = nBid and "nPBid" = nPBid and case when fIndex < nIndex then "nSerial" between fIndex and nIndex else   "nSerial" between nIndex and fIndex end;
