CREATE OR REPLACE FUNCTION public.et_realtime_export_annotations_summary(parameter json, ref1 refcursor, ref2 refcursor)
...
select * from public.et_realtime_export_annotations_summary ('{"nCaseid":"e0cd23d4-12fa-4b80-bdc0-88ec4287957b","nUserid":"eb3ec87b-7ece-43fd-919f-a9c7a77de35e","nSesid":"2b2e7d8d-609d-45f5-bce4-b574ad737cf8","cTranscript":"N","isAnnotations":true,"isHighlight":true,"jHIssues":"[]","jIssues":"[]"}','r1','r2');fetch all in "r1";fetch all in "r2";
*/
...
CASE 
  ...
  ELSE 
  ...
  )select r."nIDid" ,r."pageIndex",r."cColor",r."cONote",r."cNote", r."cLineno",r."jCordinates",jsonb_agg(distinct t) as "issues"
			from tbl r
			join dtl t on t."nIDid" = r."nIDid"
			group by r."nIDid" ,r."pageIndex",r."cColor",r."cONote",r."cNote", r."cLineno",r."jCordinates"
			order by r."pageIndex"::int
	;
	...
	case when coalesce(cTranscript,'N') = 'N' then "identity" else "tidentity" end "identity"
	From "RHighlights" rh 
	left join (
		with tbl as (
	...
	select t."nHid",case when r."cIName" ='Unassigned' then 'Quick Mark' else r."cIName" end  "cIName" --r."cIName"
	,r."cColor"
		from tbl t 
		...
		group by  t."nHid",t."cPageno",t."cLineno",t."cTime",t."cColor",t."nGroupid",t."issueids",t."cNote",t."identity"

		order by "cPageno"::int
