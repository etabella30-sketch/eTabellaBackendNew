CREATE OR REPLACE FUNCTION public.et_index_getfiles_test(parameter json, ref1 refcursor, ref2 refcursor, ref3 refcursor)
...
--     );
    ...
    FROM (
        WITH RECURSIVE bdl_tree AS (
            ...
            WHERE (bm."nParentBundleid"  IS NOT DISTINCT FROM NULL) AND sm."nCaseid" = nCaseid AND bm."nSectionid" = nSectionid
        ...
        SELECT 
            ...
            t."cBundlename"::text AS "cTab", ''::text AS "cExhibitno", ''::text AS "cRefpage",
                ''::text AS "dIntrestDt", ''::text AS "cDescription", ''::text AS "cAuthor", t.sub_info,''::text kind,t."cBundletag", t."nParentBundleid",sorted_bundletag as sorted_tab,b.sorted_name
        FROM bdl_tree t
		--join "BundleDetail" b on t."nBundleid" = b."nBundleid"
       
        GROUP BY 
            t."nBundleid", t."nParentBundleid", t."cBundlename", t.sub_info, 
            t."cBundletag", sorted_bundletag, b.sorted_name
    ...
    WITH tm AS (
        ...
        FROM "BundleDetail" bd
        WHERE 
            bd."nBundleid"  IS NOT DISTINCT FROM NULL 
