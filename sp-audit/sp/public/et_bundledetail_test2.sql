CREATE OR REPLACE FUNCTION public.et_bundledetail_test2(parameter json, ref1 refcursor)
...
-- select * from public.et_bundledetail_test ('{"nSectionid":"e38b6eff-b7dc-43cb-a4f5-8253e696dec6","nBundleid":"b111023e-078c-47f4-bbf8-3a07ae037e10","pageNumber":1,"cSearch":"","cFiletype":"ALL","cSortby":"cTab","cSorttype":"ASC","nStarttabid":null,"nEndtabid":null,"jFilter":"[{\"name\":\"LINK\",\"type\":\"V\",\"value\":[\"I\"]}]","nMasterid":"ba561c55-81f5-4180-8934-2ce6dcaa096c"}','r1');fetch all in "r1";
    ...
    IF jsonb_array_length(jFilter) > 0 THEN
        BEGIN
            ...
            IF filter_string IS NOT NULL AND filter_string != '' THEN
                filter_condition := 'incomming_facts AS (
					...
					left join incomming_facts ifs on ifs."nBundledetailid" = cr."nBundledetailid"
					left join incomming_docs ids on ids."nBundledetailid" = cr."nBundledetailid"			
					where (' || filter_string || ') and
					(f."nUserid" = ''' || nMasterid || '''::uuid or bt."nUserid" = ''' || nMasterid || '''::uuid  or idl."nUserid" = ''' || nMasterid || '''::uuid or ifs."nBundledetailid" = cr."nBundledetailid"  or ids."nBundledetailid" = cr."nBundledetailid")
    ...
    WITH ar AS (
        ...
        LEFT JOIN "BDAssignment" ba ON ba."nBundledetailid" = bd."nBundledetailid" AND ba."nSectionid" = ''' || nSectionid || '''::uuid 
            AND ' || 
            CASE WHEN explicit_null THEN 'coalesce(bd."nBundleid",''00000000-0000-0000-0000-000000000000''::uuid) = ''00000000-0000-0000-0000-000000000000''::uuid' 
                 WHEN nBundleid IS NULL THEN '1=1' 
                 ELSE 'ba."nBundleid" = ''' || nBundleid || '''::uuid' 
    ...
    WHERE (ranges.s_serial IS NULL OR childOrder.serial >= ranges.s_serial) AND (ranges.e_serial IS NULL OR childOrder.serial <= ranges.e_serial)
    ORDER BY serial
