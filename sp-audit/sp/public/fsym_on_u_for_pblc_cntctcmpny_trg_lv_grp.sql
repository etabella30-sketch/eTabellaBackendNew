CREATE OR REPLACE FUNCTION public.fsym_on_u_for_pblc_cntctcmpny_trg_lv_grp()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$                                                                                                                
          ...
          case when new."nOCompanyid" is null then '' else '"' || cast(cast(new."nOCompanyid" as numeric) as varchar) || '"' end||','||
