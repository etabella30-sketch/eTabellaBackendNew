CREATE OR REPLACE FUNCTION public.get_sorted_hierarchy_file_by_tab(root_bundle_id uuid)
 RETURNS TABLE("nSerial" bigint, "nBundleid" uuid, "nBundledetailid" uuid, "cBundlename" text, "cTab" character varying)
 LANGUAGE plpgsql
AS $function$
BEGIN
    RETURN QUERY
    WITH RECURSIVE bdl_tree AS (
        SELECT bm."nBundleid",bm."nBundledetailid", bm."cFilename"::text,
               (coalesce(bm."cTab",'') || bm."cFilename") AS sub_info,bm."cTab"
        FROM "BundleDetail" bm
        WHERE bm."nBundleid" = root_bundle_id       
    ),
    numbered_parts AS (
        SELECT *,
    ...
    sorted_parts AS (
        SELECT p."nBundleid",p."nBundledetailid", p."cFilename", sub_info, 
                p."cTab", full_path, path_parts, depth,
               array_agg(padded_numeric_parts ORDER BY idx) AS sorted_numbers
        FROM padded_parts p
        GROUP BY p."nBundleid", p."nBundledetailid", p."cFilename", sub_info, 
                 p."cTab", full_path, path_parts, depth
    )
