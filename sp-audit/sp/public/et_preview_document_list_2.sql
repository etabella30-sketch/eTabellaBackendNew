CREATE OR REPLACE FUNCTION public.et_preview_document_list_2(parameter json, ref refcursor)
 RETURNS refcursor
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
   ...
   select jsonb_agg(t) into factlinks from (
	...
	and	 (case when 
	jsonb_array_length(m."jQFIssue") > 0 and jsonb_array_length(m."jFIssue") > 0 and 
	jsonb_array_length(m."jQFContact") > 0 and jsonb_array_length(m."jFContact") > 0 then
	
	(case when jsonb_array_length(m."jQFIssue") > 0 and jsonb_array_length(m."jFIssue") > 0  then ( m."jQFIssue" @> to_jsonb(i."nIssueid"::text) or   m."jFContact" @> to_jsonb(i."nIssueid"::text) ) when jsonb_array_length(m."jQFIssue") > 0 and  f."cType"  =  'QF' then m."jQFIssue" @> to_jsonb(i."nIssueid"::text)

	when jsonb_array_length(m."jFIssue") > 0 and  f."cType"  =  'F' then m."jFIssue" @> to_jsonb(i."nIssueid"::text) else false end
	)
	or  
	(case when jsonb_array_length(m."jQFContact") > 0 and jsonb_array_length(m."jFContact") > 0  then ( m."jQFContact" @> to_jsonb(c."nContactid"::text) or   m."jFContact" @> to_jsonb(c."nContactid"::text) ) when jsonb_array_length(m."jQFContact") > 0 and  f."cType"  =  'QF' then m."jQFContact" @> to_jsonb(c."nContactid"::text)
