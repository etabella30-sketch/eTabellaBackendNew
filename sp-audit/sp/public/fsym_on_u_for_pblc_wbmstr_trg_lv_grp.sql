CREATE OR REPLACE FUNCTION public.fsym_on_u_for_pblc_wbmstr_trg_lv_grp()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$                                                                                                                
          ...
          case when new."nOWebid" is null then '' else '"' || cast(cast(new."nOWebid" as numeric) as varchar) || '"' end||','||
          ...
          case when old."nOWebid" is null then '' else '"' || cast(cast(old."nOWebid" as numeric) as varchar) || '"' end||','||
