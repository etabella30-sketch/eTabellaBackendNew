CREATE OR REPLACE FUNCTION present.fsym_on_u_for_prsnt_pmhghlghts_trg_lv_grp()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$                                                                                                                
          ...
          case when new."ZnUserid" is null then '' else '"' || cast(cast(new."ZnUserid" as numeric) as varchar) || '"' end; 
                                    var_old_data := 
          case when old."nPMHid" is null then '' else '"' || replace(replace(cast(old."nPMHid" as varchar),$$\$$,$$\\$$),'"',$$\"$$) || '"' end||','||
          ...
          case when old."ZnPMHid" is null then '' else '"' || cast(cast(old."ZnPMHid" as numeric) as varchar) || '"' end||','||
