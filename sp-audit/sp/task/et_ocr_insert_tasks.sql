CREATE OR REPLACE FUNCTION task.et_ocr_insert_tasks(parameter json, ref refcursor)
 RETURNS refcursor
 LANGUAGE plpgsql
AS $function$
...
drop table if exists temp_filterd_data;
    ...
    create temp table temp_filterd_data ON COMMIT DROP as 
        with dt as (
                ...
                and b."nBundledetailid" = nBundledetailid and upper(b."cPath") LIKE '%.PDF'
            )
            -- ,
            -- filterd_data as (
            -- select * from dt
...
select * from task.et_ocr_insert_tasks ('{""queueName"":"""",""nCaseid"":1122,""nSectionid"":9088,""nMasterid"":464,""nBundledetailid"":173517,""nOcrtype"":1}','r1');fetch all in ""r1"";
...
IF NOT bIsChecked AND nDuplicateCount > 0 THEN
        OPEN ref FOR 
            SELECT -2 AS "msg", nRemainCount AS "nRemainCount", nDuplicateCount AS "nDuplicateCount", 'File already in OCR' as value;

    elsif not exists (select 1 from temp_filterd_data) then 
        OPEN ref FOR 
            SELECT -1 AS "msg", 'No File Found for OCR' as value;
    ...
    -- -- Insert into OCRMaster and get nOid
    INSERT INTO task."OCRMaster"("nTid", "nUserid", "nCaseid", "nSectionid", "nBundledetailid", "nOcrtype")
    VALUES (nTid, nUserid, nCaseid, nSectionid, nBundledetailid, nOcrtype)
    RETURNING "nOid" INTO nOid;
    ...
    OPEN ref FOR 
        SELECT nOid AS "nOid", nTid AS "nTid", nTotal AS "nTotal";
