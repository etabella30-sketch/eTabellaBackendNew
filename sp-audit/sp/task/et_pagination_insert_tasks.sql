CREATE OR REPLACE FUNCTION task.et_pagination_insert_tasks(parameter json, ref refcursor)
 RETURNS refcursor
 LANGUAGE plpgsql
AS $function$
...
IF NOT bIsChecked AND nDuplicateCount > 0 THEN
     ...
     OPEN ref FOR 
            SELECT -1 AS "msg", 'No File Found for Pagination' as value;
else 
