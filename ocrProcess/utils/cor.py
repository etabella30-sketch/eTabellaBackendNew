...
def upload_pdf_from_disk_to_s3(bucket_name, file_key, access_key, secret_key, endpoint_url, file_path):
    try:        
...
async def ocrmy_pdf(nMasterid,identifier,id,input_pdf,sharp_image,jobs,nUDid):
    ...
    try:
        ...
        process = await asyncio.create_subprocess_exec(
            ...
            '--optimize', '1',
            '--jbig2-lossy',
            '--sharp-image', str(int(sharp_image)),
            '--jobs', str(jobs),
    ...
    # Capture stderr
        ...
        upload_pdf_from_disk_to_s3(bucket_name, input_pdf, access_key, secret_key, endpoint_url, download_path)
