...
from helper import clean_snippet, segment_first_last, refine_span_with_edge_repair, near_smart, char_span_to_token_span
...
# Bounded single-anchor recovery (global, low-risk)
SINGLE_ANCHOR_RIGHT_WIN = 48   # tokens to scan right when only START anchor is found
SINGLE_ANCHOR_LEFT_WIN = 48    # tokens to scan left when only END anchor is found
SAFE_GROW_TOK = 12             # minimal token growth when recovery fails
...
def fuzzy_anchor_edges_only(
    ...
    for retry in range(max_retries):
        ...
        # Lexical gate: require the first informative (alphanumeric) expected token to appear in the matched window
        lexical_ok = True
        try:
            first_inform = next((w.lower() for w in anchor_words if re.search(r"[A-Za-z0-9]", w)), None)
            if first_inform and ps_raw is not None:
                win_tokens = [w.lower() for w in tw[ps_raw : ps_raw + n_start]]
                if first_inform not in win_tokens:
                    lexical_ok = False
        except Exception:
            lexical_ok = True
        if ps_raw is not None:
            ps = ps_raw
        if bs >= threshold and ps is not None and lexical_ok:
    ...
    if ps is None:
        ...
        for drop in range(0, max_drop + 1):
            ...
            # Lexical gate for fallback as well
            lexical_ok = True
            try:
                first_inform = next((w.lower() for w in anchor_words if re.search(r"[A-Za-z0-9]", w)), None)
                if first_inform and ps_raw is not None:
                    win_tokens = [w.lower() for w in tw[ps_raw : ps_raw + anchor_len]]
                    if first_inform not in win_tokens:
                        lexical_ok = False
            except Exception:
                lexical_ok = True
            if ps_raw is not None and bs >= threshold and lexical_ok:
                ps = ps_raw
    ...
    # Determine base token start index
    if ps is not None:
        start_idx = max(0, ps)
    else:
    ...
    # Determine base token end index (index of the last word included)
    if pe is not None:
        # Global, non-hardcoded mitigation for early end-anchor truncation:
        # include a small trailing pad of tokens after the end anchor window.
        last_word_idx = min(len(offsets) - 1, pe + max(accepted_n_end or 1, 1) - 1 + PAD_END)
    else:
    ...
    # Single-anchor bounded recovery to avoid early truncation
    try:
        # START anchor only -> extend right toward expected suffix
        if ps is not None and pe is None and len(offsets) > 0:
            ns = int(accepted_n_start or 1)
            sd = int(accepted_start_drop or 0)
            rem_start = min(len(sw), sd + ns)
            rem_tokens = sw[rem_start:]
            win_start_tok = min(len(offsets) - 1, max(0, start_idx + ns))
            win_end_tok = min(len(offsets), win_start_tok + SINGLE_ANCHOR_RIGHT_WIN)
            if rem_tokens and (win_end_tok - win_start_tok) >= 2:
                K = max(2, min(14, len(rem_tokens)))
                anchor_suffix = " ".join(rem_tokens[:K])
                # Build substring over which to search
                win_start_char = offsets[win_start_tok]
                win_end_char = text_len if win_end_tok >= len(offsets) else offsets[win_end_tok]
                sub = text[win_start_char:win_end_char]
                m = near_smart(sub, anchor_suffix, max_dist=3, min_tokens_kept=2)
                if m is not None:
                    abs_end = win_start_char + int(getattr(m, "end", 0))
                    ti, tj = char_span_to_token_span(text, offsets[start_idx], abs_end)
                    last_word_idx = max(last_word_idx, min(len(offsets) - 1, tj))
                    dbg(f"[edges-only:recover-end] extended to token {last_word_idx} via near_smart")
                else:
                    # minimal safe growth
                    last_word_idx = min(len(offsets) - 1, last_word_idx + SAFE_GROW_TOK)
                    dbg(f"[edges-only:recover-end] no match; grew to token {last_word_idx}")

        # END anchor only -> extend left toward expected prefix
        elif pe is not None and ps is None and len(offsets) > 0:
            ne = int(accepted_n_end or 1)
            prefix_end = max(0, len(sw) - ne)
            prefix_tokens = sw[:prefix_end]
            win_end_tok = max(0, pe)  # start of end-window
            win_start_tok = max(0, win_end_tok - SINGLE_ANCHOR_LEFT_WIN)
            if prefix_tokens and (win_end_tok - win_start_tok) >= 2:
                K = max(2, min(14, len(prefix_tokens)))
                anchor_prefix = " ".join(prefix_tokens[-K:])
                # Build substring to the left of end-window
                win_start_char = offsets[win_start_tok]
                win_end_char = offsets[win_end_tok] if win_end_tok < len(offsets) else text_len
                sub = text[win_start_char:win_end_char]
                m = near_smart(sub, anchor_prefix, max_dist=3, min_tokens_kept=2)
                if m is not None:
                    abs_start = win_start_char + int(getattr(m, "start", 0))
                    ti, tj = char_span_to_token_span(text, abs_start, win_end_char)
                    start_idx = min(start_idx, max(0, ti))
                    dbg(f"[edges-only:recover-start] extended to token {start_idx} via near_smart")
                else:
                    # minimal safe growth
                    start_idx = max(0, start_idx - SAFE_GROW_TOK)
                    dbg(f"[edges-only:recover-start] no match; grew to token {start_idx}")
    except Exception:
        # On any issue, keep base indices
        pass
