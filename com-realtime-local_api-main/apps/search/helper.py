...
def compute_edge_diffs(expected: str, found: str) -> dict:
    ...
    # Heuristic: avoid anchoring the END edge on a single-token match that is a
    # low-information stopword (e.g., "of", "the"). This prevents trimming to a
    # far-right occurrence of such words in unrelated trailing text.
    try:
        STOPWORDS = {
        ...
        if last.size == 1:
            tok = E[last.a] if 0 <= last.a < len(E) else None
            if tok in STOPWORDS:
                # pick the rightmost prior block whose token is informative or size > 1
                for b in reversed(blocks[:-1]):
                    if b.size > 1:
                        last = b
                        break
                    t = E[b.a] if 0 <= b.a < len(E) else None
...
def refine_span_with_edge_repair(
    ...
    try:
        ...
        if me_cur == 0 and xs_cur == 0 and ms_cur >= 2 and ti > 0 and miss_s_tokens_cur:
        ...
        if p > 0:
            ...
            for j in range(tj + 1, limit):
                if words_lower[j] == (miss_e_tokens[0] or ""):
                    ...
                    break
            if k0 is not None:
                # include any intervening tokens up to just before k0 (e.g., "and practices")
                if k0 - 1 >= tj:
                    tj = min(len(words) - 1, k0 - 1)
                # now consume from k0 onward
                p = 1
                k = k0 + 1
                morph_used2 = False
                def tok_eq_end(a: str, b: str) -> bool:
