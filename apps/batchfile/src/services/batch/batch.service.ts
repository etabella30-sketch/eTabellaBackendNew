...
export class BatchService {
    ...
    async getfiledata(body: batchdownloadReq): Promise<batchdownloadRes> {
        ...
        try {
            await this.rds.setValue(
                `batch_download:${body.nMasterid}:${body.nCaseid}`,
                JSON.stringify({
                    nSectionid: body.nSectionid,
                    cBundleids: body.cBundleids,
                    cFilename: body.cFilename,
                    column: body.column
                }),
                24 * 3600
            );
            ...
            const res = await this.db.executeRef('batchfile_getdata', body);
            if (!res.success) {
            ...
            await this.taskQueue.add('process-task', jobData);
            this.utility.emit({
                event: 'BATCH-PROGRESS',
                data: {
                    identifier: '',
                    nMasterid: body.nMasterid,
                    type: 'Q',
                    message: 'Added in queue. Please wait...'
                }
            });
            ...
            this.logService.error(`GetData response error ${error} case ${body.nCaseid} by user ${body.nMasterid}`, this.logApp);
            this.utility.emit({
                event: 'BATCH-PROGRESS',
                data: {
                    identifier: '',
                    nMasterid: body.nMasterid,
                    type: 'F',
                    message: 'Failed to create batch file. Please try again.'
                }
            });
            return { msg: -1, value: 'Failed to create batch file', error };
        }
    }
