...
// src/app/core/interfaces/variables.interface.ts
var ICON_SETS = {
  comnicn: {
    eye: "\uE017",
    eyeoff: "\uE018",
...
var BUTTON_VARIANTS = {
  solid: "text-white bg-brand border border-brand  hover:border-brand hover:shadow-[0px_4px_4px_#00000050] rounded-base active:text-brand active:bg-brand/20 active:shadow-none disabled:bg-brand/20 disabled:border-brand/0 disabled:opacity-50 disabled:text-brand disabled:pointer-events-none",
  ...
  "outlined-blue": " text-blue-on text-xs bg-white border border-blue-deactivate  hover:border-blue-hover rounded-base   disabled:opacity-30 disabled:pointer-events-none",
...
var BADGE_VARIANTS = {
  ...
  outlined: " border border-gray-800 text-gray-800 "
};
var INPUT_VARIANTS = {
  ...
  errorClasses: "border-red-500 text-sred focus:border-sred focus:shadow-[0px_0px_6px_#f85656]",
...
// src/app/core/services/variables.service.ts
var VariablesService = class _VariablesService {
  isIconSetType(type) {
    return type in this.iconSets;
  }
  getIconUnicode(name, type) {
    if (!this.isIconSetType(type)) {
      return "";
    }
    return this.iconSets[type][name] ?? "";
  }
  constructor() {
    this.iconSets = ICON_SETS;
    this.comnicn = this.iconSets.comnicn;
    this.adminicn = this.iconSets.adminicn;
    this.indicn = this.iconSets.indicn;
    this.real_icn = this.iconSets.real_icn;
    this.myfileicn = this.iconSets.myfileicn;
    this.toolicn = this.iconSets.toolicn;
    this.extra = this.iconSets.extra;
    this.present = this.iconSets.present;
    this.commonIcons = this.iconSets.comnicn;
    this.adminIcons = this.iconSets.adminicn;
    this.indicatorIcons = this.iconSets.indicn;
    this.realtimeIcons = this.iconSets.real_icn;
    this.myFilesIcons = this.iconSets.myfileicn;
    this.toolIcons = this.iconSets.toolicn;
    this.extraIcons = this.iconSets.extra;
    this.presentationIcons = this.iconSets.present;
    this.btnvarnt = BUTTON_VARIANTS;
    this.badgevarnt = BADGE_VARIANTS;
    this.inputvarnts = INPUT_VARIANTS;
    this.buttonVariants = BUTTON_VARIANTS;
    this.badgeVariants = BADGE_VARIANTS;
    this.inputVariants = INPUT_VARIANTS;
  }
  static {
    this.\u0275fac = function VariablesService_Factory(t) {
      return new (t || _VariablesService)();
    };
  }
  static {
    this.\u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _VariablesService, factory: _VariablesService.\u0275fac, providedIn: "root" });
  }
};
...
//# sourceMappingURL=chunk-D2JKPWBT.js.map
